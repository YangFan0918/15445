...
TEST(TrieDebugger, TestCase) {
  ...
  // std::cout << trie.GetRoot()->children_.size() << "???\n";
  // auto res = trie.GetRoot();
  // if (res->children_.count('9')) {
  //   std::cout << res->children_.at('9')->children_.size() << "sdasd\n";
  // } else {
  //   std::cout << "0"
  //             << "\n";
  // }
  // res = res->children_.at('9');
  // res = res->children_.at('6');
  // res = res->children_.at('9');
  // assert(res);
  // assert(res->is_value_node_);
  // auto p = std::dynamic_pointer_cast<const TrieNodeWithValue<uint32_t>>(res);
  // std::cout << *(p->value_) << "!!!\n";
