...
struct DiskRequest {
  ...
  DiskRequest() = default;
  DiskRequest(bool is_write, char *b, page_id_t page_id, std::promise<bool> d)
      : is_write_(is_write), data_(b), page_id_(page_id), callback_(std::move(d)){};
  DiskRequest(DiskRequest &&r) noexcept {
    is_write_ = r.is_write_;
    data_ = r.data_;
    // memcpy(data_, r.data_, BUSTUB_PAGE_SIZE);
    page_id_ = r.page_id_;
    callback_ = std::move(r.callback_);
  };
};
