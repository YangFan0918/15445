...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower") {
    ...
    StringExpressionType res = StringExpressionType::Lower;
    return std::make_shared<StringExpression>(args[0], res);
  }
  if (func_name == "upper") {
    if (args.size() != 1) {
      throw Exception(fmt::format("func call {} not supported in planner yet", func_name));
    }
    StringExpressionType res = StringExpressionType::Upper;
    return std::make_shared<StringExpression>(args[0], res);
  }
  throw Exception(fmt::format("func call {} not supported in planner yet", func_name));
